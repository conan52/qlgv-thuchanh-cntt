...
namespace TkbThucHanhCNTT.Models.Provider
{
    public class ChamCongProvider
    {
        ...
        {
            IList<LichThucHanh> dsLichTh = DataProvider<LichThucHanh>.GetAll(l => l.MonHoc, l => l.GiangVien1,
            ...
            foreach (LichThucHanh lth in dsLichTh)
            {
                ...
                if (soNguoiCoMat > 0)
                {
                    ...
                    if (lth.Gv1CoMat)
                        result.Add(new ChamCongViewModel
                        {
                            MaGv = lth.Gvhd1,
                            MonHocId = lth.MonHocId,
                            SoTiet = tiet,
                            TenLop = lth.TenLop,
                            TenMonHoc = lth.MonHoc.TenThucHanh,
                            TenGv = lth.GiangVien1.HoVaTen
                        });
                    if (lth.Gvhd2 != null && lth.Gv2CoMat)
                        result.Add(new ChamCongViewModel
                        {
